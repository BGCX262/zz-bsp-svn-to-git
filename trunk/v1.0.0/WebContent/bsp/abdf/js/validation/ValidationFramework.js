 ...
 function doValidate(formRef,fevf,moduleName) {
    ...
    try {
        ...
        if(typeof(FLOW_NOT_NULL_DIELDS)!="undefined")
        {
          ...
          if(SUBMIT_WF_ACTION=="submit" || SUBMIT_WF_ACTION=="save")
          {
              for(var i=0;i<FLOW_NOT_NULL_DIELDS.length;i++)
              {
                 ...
                 if(field==null)
                 {
                   ...
                   validationField.setDesc(wfField.getFieldChinaName());
                   ...
                   validation.setDisplayError("不能为空！");
...
ValidationFactory.getValidationFromDOM=function(dom){
    ...
    v.setParams(dom.getAttribute("params"));
    v.setDisplayError(dom.getAttribute("displayError"));
    if (dom.getAttribute("atClient") == null || dom.getAttribute("atClient") =="") {
         v.setAtClient(true);
    }
    else {
      v.setAtClient(dom.getAttribute("atClient"));
    }
    v.setFire(dom.getAttribute("fire"));
    v.setFireValue(dom.getAttribute("fireValue"));
 ...
 FieldFactory.getFieldFromDOM = function(dom) {
    ...
    field.setDesc(dom.getAttribute("desc"));
...
FormFactory.getFormFromDOM = function(dom) {
    ...
    if (dom.hasChildNodes()) {
        ...
        for (var i = 0; i < f.length; i++) {
          ...
          for (var i = 0; i < f.length; i++) {
            if (f.item(i) == null||typeof(f.item(i).tagName) == 'undefined' || f.item(i).tagName != 'param') {
                continue;
            }
            var param = ParamFactory.getParamFromDOM(f.item(i));
            if (param != null) {
                form.addParam(param);
                param.setForm(form);
            }
...
ValidationFramework.validateField = function(field,fevf) {
    ...
    for (var i = 0; i < validations.length; i++) {
        ...
        var errorInfo = ValidationFramework.validateValidation(field, validations[i],fevf);
        if ( errorInfo != "") {
            //retStr = ValidationFramework.getErrorString(field, validations[i]);
            retStr=field.getDesc() + errorInfo;
...
ValidationFramework.validateValidation = function(fieldP, validation,fevf) {
     if(!ValidateMethodFactory.validateHelpMethod(fieldP,fevf)) return "";
     var obj = ValidationFramework.getWebFormFieldObj(fieldP);
     var fire = validation.getFire();
     if(fire!=null && fire!="") {
        var formObj = document.getElementById(ValidationFramework.currentForm.getName());
        var fireRealValue = "";
          if (typeof(formObj[fire].type) == "undefined") {
          for (var i = 0; i < formObj[fire].length; i++) {
            if (formObj[fire][i].checked) {
                fireRealValue = fireRealValue+formObj[fire][i].value+",";
            }
           }
         ...
         else if (formObj[fire].type == "checkbox" || formObj[fire].type == "radio") {
            if (formObj[fire].checked) {
               fireRealValue = formObj[fire].value;
            }
         } else {
           fireRealValue = formObj[fire].value;
         }
         if (fireRealValue != validation.getFireValue())   return "";
     }
     var returnValue = "";
     if (!validation.getAtClient()) return returnValue;
     ...
     for(var i=0;i<obj.length;i++)
     {
        ...
        if (validation.getName() == "required") {  //必填验证
		     ...
		     {
		        if (!ValidateMethodFactory.validateRequired(obj, validation.getParams(),fevf)) {
		             return validation.getDisplayError();
		        }
		            
		     }
		     else if(obj!=null&&obj[0].type=='radio')
		     {
		        if (!ValidateMethodFactory.validateRequired(obj, validation.getParams(),fevf)) {
		            return validation.getDisplayError();
		        }
			     ...
			     if(!ValidateMethodFactory.validateRequired(field, validation.getParams(),fevf))  {
			         return validation.getDisplayError();
	              }
             }
        } 
        ...
        {
            if (!ValidateMethodFactory.validateInteger(field, validation.getParams(),fevf)) {
                return validation.getDisplayError();
            }
        }
        else if(validation.getName() == "double"||validation.getName() == "float")   //浮点数验证
        {
            if (!ValidateMethodFactory.validateDouble(field, validation.getParams(),fevf)) {
            return validation.getDisplayError();
            }
        }
        else if(validation.getName() == "date" || validation.getName() == "Date") //日期验证
        {
            if (!ValidateMethodFactory.validateDate(field, validation.getParams(),fevf)) {
            return validation.getDisplayError(); 
            }
        }
        ...
        {
            if (!ValidateMethodFactory.validateEmail(field, validation.getParams(),fevf)) {
            return validation.getDisplayError();
            }
        }
        else if(validation.getName() == "chineseChar") //中文验证
        {
            if (!ValidateMethodFactory.validateChineseChar(field, validation.getParams(),fevf)) {
            return validation.getDisplayError();
            }
        ...
        {
            if (! ValidateMethodFactory.validateChar(field, validation.getParams(),fevf)) {
            return validation.getDisplayError();
            }
        ...
        {
           if (!ValidateMethodFactory.validateEqulas(field, validation.getParams(),fevf,fieldP)) {
            return validation.getDisplayError();
            }
        ...
        {
        if (!ValidateMethodFactory.validateMinLength(field, validation.getParams(),fevf,fieldP)) {
            return validation.getDisplayError();
            }
        ...
        {
           if (!ValidateMethodFactory.validateMaxLength(field, validation.getParams(),fevf,fieldP)) {
            return validation.getDisplayError();
            }
        ...
        {
            if (!ValidateMethodFactory.validateIntegerRange(field, validation.getParams(),fevf,fieldP)) {
            return validation.getDisplayError();
            }
        ...
        {
         if (!ValidateMethodFactory.validateDoubleRange(field, validation.getParams(),fevf,fieldP)) {
            return validation.getDisplayError();
            }
         
        }
         else if(validation.getName() == "simpleAjax")  //ajax的验证
        {
         if (!ValidateMethodFactory.validateSimpleAjax(field, validation.getParams(),fevf,fieldP)) {
            return validation.getDisplayError();
            }
