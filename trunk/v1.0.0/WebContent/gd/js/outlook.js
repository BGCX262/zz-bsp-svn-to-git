...
//
// Sie d�rfen dieses Script auf Ihren Webseiten verwenden
// Sie d�rfen diesen Copyright Hinweis jedoch nicht entfernen!
// ---------------------------------------------------------------------------
...
function b_mOut_ns4(nr) {
  ...
  if (this.smallicons) {
	alert("------3-----");	 	  
    l.write("<TABLE border=100><TR><TD width=25>")
...
function b_writeStyle() {
  ...
  document.write('		font-size:12pt; cursor:hand; border-width:0;');
  ...
  //��ϸѡ���div
  document.write('.imgB {color:'+this.color+'; text-decoration:none; font-family:\''+this.buttonfont+'\'; font-size:'+this.buttonfontsize+'; padding-top:10px;padding-right:10px;height:60px;}');

  //=========�����ͼƬʱ��������ʽ=============  
  //�����ͼƬʱ onClick
  document.write('.imgbin {border-width:0; border-style:inset;border-color:'+this.bordercolor+';} ');
  //����Ƶ�ͼƬ����ʱ onMouseOver_onMouseUp
  document.write('.imgbout {border-width:0; border-style:outset;border-color:'+this.bordercolor+';}');
  //ͼƬĬ��ͼƬ onMouseOut
  document.write('.imgnob {border-width:0; border-style:solid;border-color:'+this.bgcolor+';} ');
...
var c=0;
  ...
  if (bt.ns4) {                 //draw OutlookBar for ns4
	...
	alert("------12-----");
       ...
       document.write('<Layer visibility=hide top='+(this.captionheight+12)+' left='+(this.width-30));
       document.write('><A href="#" onFocus="if(this.blur)this.blur();" onClick="'+this.panels[i].v+'.down('+(this.buttonspace / 5)+');" ');
       document.write('onmouseOver="'+this.panels[i].v+'.clear();"><img ');
       document.write('src='+baseURL+'/images/public/arrowup.gif border=0>');
       document.write('</A></LAYER>');
       document.write('<Layer top=');
       document.write((this.height-(this.panels.length-1)*this.captionheight-30)+' left=');
       document.write((this.width-30)+'><A href="#" onFocus="if(this.blur)this.blur();" onClick="');
       document.write(this.panels[i].v+'.up('+(this.buttonspace/5)+');" onmouseOver="');
       document.write(this.panels[i].v+'.clear();"><img ');
       document.write('src='+baseURL+'/images/public/arrowdown.gif border=0></A></LAYER>');
    ...
    for (i=0;i<this.panels.length;i++) {
      ...
      for (j=0;j<this.panels[i].img.length;j++) {
        if (bt.op5) {
          ...
          //alert("------18-----");
          document.write('<DIV id='+this.name+'_panel'+i+'_b'+j+' class=imgB onClick="onclicktd(this.id)"');
          ...
          if (this.smallicons) {
			...
			//��ʾ��ϸ
            document.write('; top:'+mtop+'; text-align:center;">');
            document.write('<A onFocus="if(this.blur)this.blur();" href="'+ this.panels[i].act[j].substring(35,this.panels[i].act[j].length-1) + '" target="right">' );
            document.write('<img  src='+this.panels[i].img[j]+' class=imgnob ' );
            document.write(' onmouseOver="this.className=\'imgbout\';" ');
            document.write('onmouseDown="this.className=\'imgbin\';" ');
            document.write('onmouseUp="this.className=\'imgbout\';" ');
            document.write(' onmouseOut="this.className=\'imgnob\';"></a><BR>');
      ...
      if (!bt.op5) {
        ...
        document.write('><TR><TD align=center border=0 valign=middle style=\'background-image:url('+baseURL+'/images/public/buttomTree.gif) \'>');
      ...
      document.write('<img src='+baseURL+'/images/public/arrowup.gif border=0>');
      ...
      document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
      document.write('<img src='+baseURL+'/images/public/arrowdown.gif border=0>');
      document.write('</A></DIV>');
...
*/
var oldRow="";			//�洢֮ǰ����IDֵ
function onclicktd(r){
	//oldRow!=""��֮ǰ�ѱ�ѡ��
	if(oldRow!="" & oldRow!=r){			
		//document.all(oldRow).style.color='#000000';
		document.all(oldRow).style.backgroundImage="";
	}	
	document.all(r).style.backgroundImage="url("+baseURL+"/images/public/treeBg.gif)";
	oldRow=r;
